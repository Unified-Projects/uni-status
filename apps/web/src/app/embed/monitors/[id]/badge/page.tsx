...
export default async function MonitorEmbedBadgePage({ params, searchParams }: PageProps) {
  const { id } = await params;
  const { label = "status", style = "flat" } = await searchParams;

  // Fetch the SVG badge from the API
  const badgeUrl = `${API_URL}/api/public/embeds/monitors/${id}/badge.svg?label=${encodeURIComponent(label)}&style=${style}`;
